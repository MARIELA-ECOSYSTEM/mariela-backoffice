...
vi.mock("@tanstack/react-router", async (importOriginal) => {
  ...
  return {
    ...
    useRouter: () => ({ state: { location: { pathname: "/campanhas" } } }),
    // `PeriodoCard` (compartilhado com Coleções) usa `Link` de verdade para
...
function campanha(overrides: Partial<Campanha> & { id: string; nome: string }): Campanha {
  return {
    codigo: `CAM-${overrides.id}`,
    descricao: "",
    inicio: "2026-01-01",
    fim: "2026-12-31",
    ativo: true,
...
/** Base de 30 campanhas — grande o bastante para exigir 3 páginas de 12 (POR_PAGINA da tela). */
const BASE = Array.from({ length: 30 }, (_, indice) =>
  campanha({ id: String(indice + 1), nome: `Campanha ${String(indice + 1).padStart(2, "0")}` }),
);

const listarMock = vi.fn();
vi.mock("@/services/api/cadastros.api", () => ({
  PAGINA_PADRAO_CAMPANHAS: 1,
  LIMITE_PADRAO_CAMPANHAS: 20,
...
function respostaPaginada(filtros: CampanhaFiltros) {
  ...
  return {
    campanhas: filtrados.slice(inicio, inicio + limit),
...
const CampanhasPage = Route.options.component!;
...
describe("Tela de Campanhas — paginação e filtros server-side", () => {
  afterEach(cleanup);

  beforeEach(() => {
    vi.clearAllMocks();
    listarMock.mockImplementation(async (filtros: CampanhaFiltros = {}) =>
  ...
  it("carrega a primeira página com page=1 e limit=12 (POR_PAGINA da tela) por padrão", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Campanha 01")).toBeTruthy());
    expect(listarMock).toHaveBeenCalledWith(expect.objectContaining({ page: 1, limit: 12 }));
    expect(screen.getByText(/30 campanha\(s\) encontrada\(s\)/)).toBeTruthy();
  ...
  it("próxima página: chama a API com page=2 e mostra os itens seguintes", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Campanha 13")).toBeTruthy());
    expect(screen.getByText(/página 2 de 3/)).toBeTruthy();
    expect(screen.queryByText("Campanha 01")).toBeNull();
  });
  ...
  it("mudar a busca volta para a página 1 e refaz a consulta", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Buscar por nome, descrição/), {
      target: { value: "Campanha 05" },
    });

    await waitFor(() =>
      expect(listarMock).toHaveBeenLastCalledWith(
        expect.objectContaining({ page: 1, busca: "Campanha 05" }),
      ),
    );
  ...
  it("resposta vazia mostra o estado vazio, não uma grade em branco", async () => {
    ...
    await waitFor(() => expect(screen.getByText("Nenhuma campanha encontrada")).toBeTruthy());
  });
  ...
  it("indica atualização em segundo plano ao trocar de página, sem sumir com a lista atual", async () => {
    ...
    expect(screen.getByText("Campanha 01")).toBeTruthy();
