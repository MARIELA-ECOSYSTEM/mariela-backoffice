...
/**
 * Conteúdo do menu lateral, compartilhado entre a sidebar fixa (desktop)
 * e o drawer off-canvas (telas estreitas). Itens, ordem, grupos, ícones,
 * rotas e estados ativos são exatamente os mesmos nos dois contextos.
 */
export function SidebarConteudo({ onNavegar }: { onNavegar?: () => void }) {
  return (
    <div className="flex h-full min-h-0 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <nav aria-label="Menu principal" className="min-h-0 flex-1 overflow-y-auto px-4 pb-6">
              ...
              {grupo.itens.map((item) => (
                <li key={item.to}>
                  <Link
                    to={item.to}
                    onClick={onNavegar}
                    className="group relative flex items-center gap-3 rounded-md px-3 py-2 text-sm text-sidebar-foreground/70 transition-colors duration-200 hover:bg-sidebar-accent/70 hover:text-sidebar-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-primary/60"
                    activeProps={{
                      className:
